...
const baseSchema = z.object({
  ...
  especie_documento: z.enum(['CONTAS_A_PAGAR', 'NOTA_FISCAL', 'FATURA', 'DUPLICATA', 'BOLETO', 'RECIBO', 'NOTA_FISCAL_SERVICO', 'PEDIDO', 'ORDEM_COMPRA', 'OUTROS']).optional().nullable(),
  ...
  data_pagamento: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().nullable().or(z.literal('')),
...
const transformEmptyStrings = (data: any) => {
  return {
    ...
    data_pagamento: data.data_pagamento === '' ? undefined : data.data_pagamento,
...
// GET /contas-pagar - Listar todas as contas a pagar
router.get('/', async (req, res) => {
  try {
    ...
    console.error('Erro no endpoint GET /contas-pagar:', error);
    res.status(500).json({ error: 'Erro interno do servidor' });
  }
});

// GET /contas-pagar/proximo-numero - Buscar próximo número sequencial por espécie
router.get('/proximo-numero', async (req, res) => {
  try {
    ...
    console.error('Erro no endpoint GET /contas-pagar/proximo-numero:', error);
    res.status(500).json({ error: 'Erro interno do servidor' });
  }
});

// GET /contas-pagar/:id - Buscar uma conta específica
router.get('/:id', async (req, res) => {
  try {
    ...
    console.error('Erro no endpoint GET /contas-pagar/:id:', error);
    res.status(500).json({ error: 'Erro interno do servidor' });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!parse.success) {
      ...
      return res.status(400).json({ 
        ...
        formato_esperado: {
          ...
          valor_pago: 'number >= 0 (opcional, padrão: 0)',
          ...
          data_pagamento: 'string no formato YYYY-MM-DD (opcional)',
    ...
    console.error('Erro no endpoint POST /contas-pagar:', error);
    res.status(500).json({ error: 'Erro interno do servidor' });
  }
});
...
router.put('/:id', async (req, res) => {
  try {
    ...
    console.error('Erro no endpoint PUT /contas-pagar/:id:', error);
    res.status(500).json({ error: 'Erro interno do servidor' });
  }
});

// DELETE /contas-pagar/:id - Soft delete da conta a pagar
router.delete('/:id', async (req, res) => {
  try {
    ...
    console.error('Erro no endpoint DELETE /contas-pagar/:id:', error);
    res.status(500).json({ error: 'Erro interno do servidor' });
  }
});

// POST /contas-pagar/:id/pagar - Marcar como pago
router.post('/:id/pagar', async (req, res) => {
  try {
    ...
    console.error('Erro no endpoint POST /contas-pagar/:id/pagar:', error);
    res.status(500).json({ error: 'Erro interno do servidor' });
  }
});
